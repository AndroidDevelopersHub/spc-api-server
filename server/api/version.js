...
module.exports = function (router) {
    ...
    router.get('/version/:id', details);
    router.delete('/version/:id', _delete);
}
...
function add(req, res){
    ...
    db.query("INSERT INTO version SET ?", req.body , (err, result) => {
        if (!err) {
...
async function list(req ,res ){
    ...
    db.query("SELECT COUNT(*) AS total FROM version", (err, result) => {
        if (!err) {
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM version WHERE CONCAT(title) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
        ...
        db.query("SELECT * FROM version LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
            if (!err) {
                return _response.apiSuccess(res, result.length+" "+responsemsg.found , result , {page: parseInt(page) , limit: parseInt(limit),totalDocs: totalDocs })
...
function update(req ,res ){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `version` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {

                db.query("UPDATE version SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `version` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!result.length){
                ...
                db.query("DELETE FROM `version` WHERE id='" + req.params.id + "'", (err, result) => {
                    if (!err) {
