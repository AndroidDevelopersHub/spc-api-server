...
module.exports = function (router) {
    router.get('/registration-package', list);
    router.post('/registration-package', add);
    router.put('/registration-package/:id', update);
    router.get('/registration-package/:id', details);
    router.delete('/registration-package/:id', _delete);
...
function add(req, res){
    ...
    db.query("INSERT INTO registration_package SET ?", req.body , (err, result) => {
        if (!err) {
...
async function list(req ,res ){
    ...
    db.query("SELECT COUNT(*) AS total FROM registration_package", (err, result) => {
        if (!err) {
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM registration_package WHERE CONCAT(title) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
        ...
        db.query("SELECT * FROM registration_package LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
            if (!err) {
                return _response.apiSuccess(res, result.length+" "+responsemsg.found , result , {page: parseInt(page) , limit: parseInt(limit),totalDocs: totalDocs })
...
function update(req ,res ){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `registration_package` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {

                db.query("UPDATE registration_package SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `registration_package` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!result.length){
                ...
                db.query("DELETE FROM `registration_package` WHERE id='" + req.params.id + "'", (err, result) => {
                    if (!err) {
